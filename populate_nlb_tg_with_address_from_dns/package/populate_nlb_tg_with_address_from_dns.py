...
logger.info("SERVICE_HOSTNAME is %s." % service_hostname)
...
def get_node_ip_from_dns(dns_name, record_type, total_retry_count):
    ...
    :param dns_name: DNS name of Service
...
def get_ip_from_dns():
    ...
    ip_from_dns_set = get_node_ip_from_dns(
        service_hostname, "A", max_lookup_per_invocation
    )
    logger.info(
    ...
    try:
        error_message = (
            f"No IP found from DNS for - {service_hostname}. "
...
def get_ip_target_list_by_target_group_arn(tg_arn):
    ...
    logger.info(
        f"Service IPs that are currently registered with the target group: {registered_ip_list}. "
...
def handler(event, context):
    ...
    logger.info(
        f"Service IPs from target group ({nlb_tg_arn}): {ip_from_target_group_set}. "
    ...
    if len(ip_from_dns_set) == 0:
        logger.info(f"No DNS Entry found for : {service_hostname}")
